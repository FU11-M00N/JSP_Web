...
$(document).ready(function() {
	var myLatlng = new google.maps.LatLng(35.877589,127.778883); // 위치값 위도 경도
	var Y_point			= 35.877589;		// Y 좌표
	var X_point			= 127.778883;		// X 좌표
	var zoomLevel		= 17;				// 지도의 확대 레벨 : 숫자가 클수록 확대정도가 큼
	var markerTitle		= "구천동";		// 현재 위치 마커에 마우스를 오버을때 나타나는 정보
...
<body>
	...
	<div>
		...
		<table class="admin2">
			<tr>
				<td align="left" class="board2">구천동</td>
			</tr>
		</table>
		<table class="admin">
			<tr>
		...
		<div class="slideshow">
		     <img src=".\..\img\구천동.jpg" width="500" height="250"/>		     
		</div>
		<div class="board2"><br>
			설천면(雪川面) 소천리(小川里)에 있는 나제통문(羅濟通門), 즉 신라와 백제의 경계관문이었던 석굴문(石掘門)에서 덕유산 상봉에 이르는 25km의 계곡으로, <br>
			33경(景)으로 꼽히는 계곡미가 뛰어나 덕유산국립공원의 중심부를 이룬다. 구절양장(九折羊腸) 9,000굽이를 헤아린다는 계곡에 학소대(鶴巢臺)·추월담(秋月潭) <br>
			·수심대(水心臺)·수경대(水鏡臺)·인월담(印月潭)·청류동(淸流洞)·구월담(九月潭)·금포탄(琴浦灘)·청류계(淸流溪)·구천폭포(九千瀑布)등 나제통문을 <br>
			제1경으로 하여 덕유산 상봉을 제33경으로 하는 절경들이 줄을잇는다 덕유산 정상부 가까이에 있는 백련사(白蓮寺)는 구천동 골짜기에 있는 유일한 사찰로 주변경치가 매우 수려하다 <br><br>
	
			위치: 전북 무주군 설천면 장덕리 구천동
		</div>
		</div>
</body>
</html>