...
<body>
		...
		<ul class="list4">
			...
			<li class="login">│</li>
			<li class="joinus"><a href="mypagemain.jsp?id=<%=session.getAttribute("id") %>" class="aa">My Page</a></li>
			<li class="joinus">│</li>
		...
		<div class="full">
			...
			%>
				<tr>
					...
					<td><%=area %></td>
					<td><%=createtime %></td>
					<td><input type="button" class="button" value="수정" onClick="location.href='mypageupdate.jsp?id=<%=rs.getString("id") %>'">
						<input type="button"class="button"  value="탈퇴" onClick="location.href='mypagedeleteJS.jsp?id=<%=rs.getString("id") %>&password=<%=rs.getString("password") %>'"></td>
				</tr>
			<%	}	
			}catch(Exception e){
